    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
        ...
        <div className="glass-card" style={{ padding: '20px' }}>
          <div style={{ fontSize: '13px', fontWeight: '600', color: '#E6EDF3', marginBottom: '16px' }}>Receivables</div>
          {invoices.filter((i) => i.status !== 'PAID').length === 0 ? (
            ...
            <table className="audit-grid">
              <thead>
                <tr>
                  <th>Client</th><th>Ref</th><th className="numeric">Amount</th><th>Status</th>
                </tr>
              </thead>
              <tbody>
        ...
        {/* Audit Trail */}
        <div className="glass-card" style={{ padding: '20px' }}>
          <div style={{ fontSize: '13px', fontWeight: '600', color: '#E6EDF3', marginBottom: '16px' }}>Recent Pipeline Runs</div>
              ...
              {incomeEvents.map((ev) => (
                <div key={ev.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid #21262D' }}>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    ...
                    <div style={{ fontSize: '10px', color: '#F59E0B', marginTop: '2px' }}>Tax: {formatINR(ev.tax_slice)}</div>
                  </div>
                </div>
              ))}
            </div>
